...
// Only completed workouts inside the phase are counted and repeated exercise performance is compared chronologically.
context.workoutHistory=[session('2026-08-31',[ex('chest',200,10)]),session('2026-09-02',[ex('chest',100,10),ex('row',100,10)]),session('2026-09-20',[ex('chest',110,10),ex('row',90,10)]),session('2026-09-25',[ex('chest',120,10)])];
...
// Finish validation rejects a date before the phase start. Finishing on Sep 24 excludes the Sep 25 session.
assert.equal(context.prismPhaseFinish(190,'2026-08-30'),false);assert.ok(context.prismActivePhase());assert.equal(context.prismPhaseFinish(190,'2026-09-24'),true);assert.equal(context.prismActivePhase(),null);let phases=context.prismPhaseRead();assert.equal(phases[0].endWeight,190);assert.equal(phases[0].endDate,'2026-09-24');assert.equal(context.prismPhaseAnalyze(phases[0]).training.workouts,2);assert.equal(context.prismPhaseAnalyze(phases[0]).weightChange,-10);
// A new phase can begin after the prior phase is finished.
assert.equal(context.prismPhaseAdd('bulk','2026-09-25',190,8),true);assert.equal(context.prismActivePhase().type,'bulk');
// Invalid goal types never write data.
const beforeInvalid=JSON.stringify(context.prismPhaseRead());assert.equal(context.prismPhaseAdd('invalid','2026-09-25',190,8),false);assert.equal(JSON.stringify(context.prismPhaseRead()),beforeInvalid);
// Comparison describes differences without mutating phase history.
context.workoutHistory.push(session('2026-09-25',[ex('chest',120,10)]),session('2026-09-26',[ex('chest',125,10)]));context.prismPhaseFinish(195,'2026-09-26');phases=context.prismPhaseRead();const snapshot=JSON.stringify(phases);const comparison=context.prismPhaseCompare(phases[0],phases[1]);assert.equal(comparison.a.label,'Cut');assert.equal(comparison.b.label,'Bulk');assert.equal(JSON.stringify(context.prismPhaseRead()),snapshot);
