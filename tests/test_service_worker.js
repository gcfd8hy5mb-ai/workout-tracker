...
assert.match(fs.readFileSync("index.html","utf8"),/<script src="onboarding\.js\?v=10\.3-beta5"><\/script>/,"HTML and offline cache must use the same release URL");
...
let claimed=false;
const cache={
  async addAll(files){assert.deepEqual(Array.from(files,request=>request.url),["./","./index.html","./onboarding.js?v=10.3-beta5","./pro-experience.js","./manifest.json","./sw.js"]);assert.equal(files.every(request=>request.cache==="reload"),true);entries.set("shell",true)},
...
const caches={open:async()=>cache,keys:async()=>["prism-v10.3-beta4","prism-v10.3-beta5"],delete:async key=>{deleted.push(key)}};
...
assert.deepEqual(deleted,["prism-v10.3-beta4"]);
